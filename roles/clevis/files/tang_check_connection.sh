...
if [ -z "$SLOTS_INFO" ]; then
    ...
    echo "Have you run 'clevis luks bind'?"
    exit 1
fi

echo -e "Bindings found:\n$SLOTS_INFO\n"

# Iterate through slots and test each one
while IFS= read -r line; do
    # Use awk to safely split the fields.
    # $1 is the slot (e.g. "1:"), $2 is the pin (e.g. "sss")

    # Slot number: take first field and strip the colon
    ...
    if [ "$PIN" == "tang" ] || [ "$PIN" == "sss" ]; then
        ...
        # The actual test:
        START_TIME=$(date +%s%N)
        if timeout 15s clevis luks pass -d "$DEVICE" -s "$SLOT" > /dev/null; then
            END_TIME=$(date +%s%N)
